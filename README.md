...
- **Targets (URL → actions)** — a JSON file of named recipes: an exact or prefix URL match, then ordered steps whose post-conditions must hold before the document is read. The longest match wins; two equally specific matches are a configuration error; a URL that matches nothing is fetched exactly as before. The first verb is `waitFor` (visible text appearing or disappearing, a URL, or a fixed wait); a step that does not hold stops the fetch with `WEB_FETCH_ACTION` instead of reading a page the target never reached, and when a target runs the body opens with a one-line summary of what ran and where it ended:
  ```json
