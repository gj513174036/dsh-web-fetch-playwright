...
- **The DevTools port stays on loopback.** `--address` accepts only `127.0.0.1`, the rest of `127.0.0.0/8`, `::1`, or `localhost`; anything else is refused with the reason (the port is full control of the browser *and* the logins in its profile). Remote access is the reverse tunnel's job.
