...
Explicit exceptions (not covered): a hop's extra arriving **after a later hop's** extra when neither carries a usable identity — the two are locally indistinguishable; and a response extra whose status never matches any hop, which stays unclaimed and is flushed with `unclaimed: true` after a short hold window. The two `har.json` and `network.jsonl` artifacts agree on every case above (both are fed to `tools/netdump` in the project's checks).
