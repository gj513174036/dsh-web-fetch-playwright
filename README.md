...
- **Targets (URL → actions)** — a JSON file of named recipes: an exact or prefix URL match, then ordered steps whose post-conditions must hold before the document is read. The longest match wins; two equally specific matches are a configuration error; a URL that matches nothing is fetched exactly as before. `waitFor` waits for visible text appearing or disappearing, a URL, a fixed time, a **state** over a candidate list (`all checked` / `all unchecked` / `all enabled` / `all disabled`) — the one a gate needs, whose requirement lives in state and in no label — or a **response** whose URL matches (the same match clause a target carries): the direct "the data has arrived" signal, watched from the moment the actions start, so a response the page the run starts on fetched before the run cannot satisfy a wait (the page an `opensPage` click opens is watched from the moment it opens, so its own loading traffic is inside the window — that page *is* the document the fetch reads). A response is an event rather than a state: a wait spends every matching arrival in hand (so a duplicate from a double-fetch cannot answer the next wait), only the page the run is on answers (not the one an `opensPage` step left), a main-frame document or a 4xx/5xx never counts as the data arriving (though both are named in a failure), and a click is credited only with a response from a request that started **after** the click — a response already in flight when it went out proves nothing, as does one an earlier wait spent — with a wait confirming only the act immediately before it. `click` names an intent and an ordered candidate list (a CSS selector, a visible text, or a role with an accessible name) and clicks the first *reachable* one — reachability (exists, laid out, not covered, not disabled) is verified before the click, and a candidate list that runs out fails the fetch with the step, the candidates and the URL. A click marked `"opensPage": true` is expected to open a tab (`target="_blank"`, `window.open`): the run waits for it, continues on it, and the fetch reads *that* document; no tab, and the step fails instead of reading the page it stayed on. The click does not claim its own effect: the `waitFor` after it is what proves the page moved, and a click no later `waitFor` confirmed is marked `clicked (unverified)` in the summary. `type` writes a value into the first reachable field that takes text and makes the page hear it (focus, the prototype's value setter, `input` + `change`), replacing what was there and reading it back so a page that reverted it fails loudly instead of searching with an empty box. `check` satisfies a precondition by putting a control into a state the way a person does it — through the control itself, or the `<label>` that forwards a click to it — and passes only when the control *reports* that state afterwards; a control already in it is left alone. A step that does not hold stops the fetch with `WEB_FETCH_ACTION` instead of reading a page the target never reached, and when a target runs the body opens with a one-line summary of what ran and where it ended:
  ...
  ```
- **Observe mode** — returns what a page is *asking for* (controls, labels, states, counts) rather than its prose, for the case where a gate's requirements are nowhere in its text. See the `observe` setting below.
...
- **Budget-aware** — per-fetch deadline (45s by default, `fetchBudgetMs`); concurrency is backend-priced (`maxConcurrency`, default 4 local browsers / **50 tabs** for the CDP and DSH-managed backends; queued fetches fail fast with a retry hint after 20s instead of hanging); image/font/media subrequests aborted; body capped at 100k chars.
- **Bounded Cloudflare-challenge wait** — when a navigation lands on a challenge interstitial ("Just a moment…" and its localized siblings, recognized via the documented `cf-mitigated: challenge` response header plus structural page markers), the fetch keeps the **same tab and context** and waits for the browser's own verification to clear it — tracking the *last* main-frame response (the real page reloads in) and watching the live DOM so SPA-style clears are caught too. Bounded and configurable (`challengeWaitMs`, default 15s; 0 restores the legacy first-response behavior), with a bounded same-tab retry (`challengeRetries`, default 1). When the budget runs out, the fetch fails with the distinct `WEB_FETCH_CHALLENGE` error code instead of returning the interstitial as content. By the same principle, a consent gate that survives its own dismissal fails the fetch with `WEB_FETCH_CONSENT` rather than being returned as the page. It never clicks, never injects CAPTCHA answers, never fakes browser state, and never exports or copies cookies.
...
| `targetsFile` | *(blank)* | Path to a JSON file of named recipes for URLs that need actions before their document means anything. Explicit path (the plugin knows the process working directory, not this session's workspace). Keep it in the repo — recipes are reviewed and diffed — and keep credentials out of it. See the shape in the feature list above. |
| `observe` | `false` | Return the page's actionable state instead of its text: reachable controls with labels and states (`checked`/`unchecked`, `disabled`, `covered`, visible-as-its-label), complete counts, and the head of the visible text. For meeting an unfamiliar page — the precondition no label mentions is often a count — and it is a mode, not a per-fetch option, because the fetch seam carries only a URL. |
| `dismissConsent` | `false` | Click the "accept all" control of a known consent manager (OneTrust, TrustArc, Cookiebot, Didomi, Osano, Usercentrics, CookieYes, Complianz, Iubenda, Klaro, Google Funding Choices, Quantcast) after the page settles and before it is read — and, when none of those matched, any control labelled "accept all" / 全部接受 that sits in consent-looking UI (a dialog, a consent-named ancestor, a fixed/sticky overlay, or — for a full-page consent interstitial, whose accept control has none of those — the document itself, while it is short and its URL or title names consent). Off by default: the click records *your* consent in whichever profile the fetch uses — the real one on the CDP and DSH-managed backends — and the consent cookies stay there. Best effort: a page with no banner, a backend without `evaluate`, and a click that throws all leave the fetch's outcome alone. |
...
| `fetchBudgetMs` | `45000` | The whole-fetch budget (ms, 5000–600000). Queueing, opening the browser or tab, navigation, the target's actions, a `waitFor` chain, a challenge wait and the content read all have to fit inside it. Raise it when a recipe has to outlast a slow single-page app, or when a human is expected to drive the tab while the fetch records; the tool layer's own `fetchTimeoutMs` (30s by default) still cuts the call first unless that is raised too. |
...
| `challengeRetries` | `1` | Same-tab re-navigation attempts after a wait window runs out (0–3); any clearance cookies the browser earned stay in the context for the retry. Everything stays inside the per-fetch budget (`fetchBudgetMs`, 45s by default). |
...
| File | What it is |
| --- | --- |
| `network.jsonl` | One JSON object per line, appended **while the fetch runs** — a `session` header, then `request` / `response` / `responseBody` / `finished` (or `failed`) per exchange, `requestExtra` / `responseExtra` rows carrying the authoritative header+cookie sets paired per HOP by the hop's own identity (`:path` + `:authority` for a request, the status for a response) — a redirect hop closes as its own `response`/`finished` pair, so 301→200 yields two records, and each keeps its own cookies/headers **even when a hop has no extra-info event of its own** (a cached 301, which CDP explicitly allows) or when the extra-info arrives before its hop's base event. See the pairing scope and its exceptions below, plus `websocketCreated` / `websocketFrame` / `websocketClosed`. Live-readable, crash-tolerant, and what the offline pipeline consumes. One malformed URL or record degrades to that record alone: the export never loses the rest of the document. |
...
**How per-hop pairing is scoped** (the same statement lives in `src/recorder.ts`). Verified against synthetic CDP event streams — every order below is exercised by the test suite, not by a real browser (this environment has no launchable Chromium):

- every hop with its own extra-info event, in any arrival order relative to its base event;
- a hop with **no** extra-info event at all (a cached 301) — the next hop's credentials stay on the next hop;
- a response extra-info arriving before its hop's `responseReceived` (held, then placed once that hop's status is known).

Fallback and its limits: when an extra carries no usable identity (no `:path`/`:authority`, or no status — a minimal CDP implementation or a synthetic event) or several hops match it equally, the recorder falls back to arrival order ("the Nth extra of a requestId belongs to its Nth hop", the rule Playwright itself uses). That is correct only while every earlier hop of that requestId produced its own extra of the same kind.

Explicit exceptions (not covered): a hop's extra arriving **after a later hop's** extra when neither carries a usable identity — the two are locally indistinguishable; a response extra whose status never matches any hop, which stays unclaimed and is flushed with `unclaimed: true` after a short hold window; and, on the response side, only the status code is an identity, so **consecutive hops that share a status** (an `http→https→www` chain of 301s) misattribute: when an earlier hop has no responseExtra of its own, a later hop's responseExtra — and the `Set-Cookie` it carries — lands on that earlier hop. What keeps the response side safe is distinct statuses, not hop order. The two `har.json` and `network.jsonl` artifacts agree on every case above (both are fed to `tools/netdump` in the project's checks).
...
Security boundary (deliberate): no clicking through Turnstile, no CAPTCHA solving or token injection, no fingerprint/UA spoofing, and no proxy **rotation** (the configured proxy is a single static egress hop — it is never swapped per request to dodge a challenge) — and no cookie export: in isolated mode the clearance a fetch's browser earns dies with that fetch's context; in profile mode it stays in the browser's own profile, which this plugin never copies or cleans. The wait is always bounded by `challengeWaitMs` and the per-fetch budget (`fetchBudgetMs`); nothing blocks forever.
