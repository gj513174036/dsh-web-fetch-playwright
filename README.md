...
| `dismissConsent` | `false` | Click the "accept all" control of a known consent manager (OneTrust, TrustArc, Cookiebot, Didomi, Osano, Usercentrics, CookieYes, Complianz, Iubenda, Klaro, Google Funding Choices, Quantcast) after the page settles and before it is read. Off by default: the click records *your* consent in whichever profile the fetch uses — the real one on the CDP and DSH-managed backends — and the consent cookies stay there. Best effort: a page with no banner, a backend without `evaluate`, and a click that throws all leave the fetch's outcome alone. |
