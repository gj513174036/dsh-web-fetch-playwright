...
| Loopback DevTools port | The debug port is exposed to a network | The launcher defaults to loopback and **only accepts a loopback `--address`** (`127.0.0.1`, `127.0.0.0/8`, `::1`, `localhost`): the port grants full control of the browser AND access to the logins in its profile, so it is never bound to a network interface. Remote access goes through a reverse tunnel (`autossh -R`), and keeping that tunnel loopback-bound is the operator's responsibility |
